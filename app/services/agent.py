...
class AgentService:
    ...
    def _get_generate_config(
        ...
        if user_location and user_location.get("city"):
            loc_lines.append(f"- Ubicación actual del usuario (GPS/Chat): {user_location['city']}.")
        elif user_location and user_location.get("latitude") is not None and user_location.get("longitude") is not None:
            loc_lines.append(f"- Coordenadas GPS actuales del usuario: Latitud {user_location['latitude']}, Longitud {user_location['longitude']}.")
        elif user_city:
            loc_lines.append(f"- Ciudad habitual de residencia del usuario (guardada en su perfil): {user_city}.")
        else:
            loc_lines.append(
                "- Ubicación física del usuario: NO PROPORCIONADA AÚN.\n"
                "- REGLA ESTRICTA DE CLIMA Y UBICACIÓN: 'America/Bogota' es ÚNICAMENTE el huso horario del servidor (UTC-5), "
                "NUNCA asumas que el usuario vive en Bogotá. Si el usuario pide el clima o su Daily Briefing sin especificar ciudad, "
                "pregúntale amablemente en qué ciudad se encuentra y ofrécete a recordarla con `set_user_location` para que no tenga que volver a indicarla."
            )
        loc_info = "\n".join(loc_lines)
        ...
        return types.GenerateContentConfig(
            ...
            system_instruction=(
                ...
                f"### CONTEXTO TEMPORAL Y GEOGRÁFICO:\n"
                f"- Fecha y hora actual del sistema: {current_date_str} (Huso horario de referencia: UTC-5, America/Bogota).\n"
                ...
                f"### AUDITORÍA DE AGENDA Y DAILY BRIEFING EN TIEMPO REAL:\n"
                f"- Ten SIEMPRE en cuenta la hora exacta de la consulta ({now.strftime('%H:%M')}) al auditar la agenda o buscar huecos libres de 'hoy'.\n"
                f"- Cuando el usuario pregunte algo como 'Audita mi agenda de hoy y dime qué huecos de trabajo libre tengo disponibles' o solicite su 'Daily Briefing matutino':\n"
                f"  1. Los horarios o bloques libres anteriores a la hora actual ({now.strftime('%H:%M')}) YA PASARON. Aclara explícitamente que ya no se puede apartar tiempo para esos horarios ya que ya pasaron.\n"
                f"  2. Si para el resto del día de hoy ya no quedan huecos libres disponibles (o la jornada laboral ya concluyó por la hora tardía), infórmaselo con empatía y claridad e inmediatamente ofrece o sugiere auditar y revisar huecos para el día siguiente (mañana) si el usuario lo desea.\n"
                f"  3. Si aún quedan huecos libres hoy a partir de la hora actual, enfócate en los que efectivamente se pueden aprovechar ahora, señalando brevemente que los horarios previos ya concluyeron.\n"
                f"  4. Si la agenda reporta conflictos (`has_conflicts=True`), adviértele de inmediato qué reuniones se cruzan en horario para que pueda tomar decisiones.\n"
                f"  5. Resalta las reuniones clave (`key_meetings`), como videollamadas con enlace, demos o reuniones con clientes.\n\n"
                ...
                f"1. AUDITORÍA DE AGENDA, BALANCE DE TIEMPO Y DAILY BRIEFING:\n"
                f"   - Consultar la agenda real de Google Calendar para cualquier día usando `get_calendar_agenda`. Detecta y alerta si existen solapamientos entre reuniones (`conflicts`) y resalta las reuniones prioritarias (`key_meetings`).\n"
                f"   - Detectar huecos y momentos de calma disponibles usando `find_free_work_slots` (teniendo en cuenta la hora actual de la consulta y smart buffers de descanso entre reuniones).\n"
                ...
                f"3. PERSONALIZACIÓN Y PERFIL DE USUARIO:\n"
                f"   - Memorizar la ciudad de residencia del usuario con `set_user_location` si el usuario te indica dónde vive o se encuentra (ej. 'Vivo en Medellín' o 'Guarda mi ciudad: Cali').\n"
                f"4. INVESTIGACIÓN Y APRENDIZAJE:\n"
                ...
                f"5. BIENESTAR Y CLIMA:\n"
                f"   - Consultar el clima con `get_weather`. Si conoces la ciudad por las coordenadas o el perfil del usuario, consúltala directamente. Si no la conoces, NUNCA asumas Bogotá: pregúntale amablemente en qué ciudad se encuentra.\n\n"
                ...
                f"3. NO PUEDES INVENTAR DISPONIBILIDAD NI CIUDADES: Jamás asumas que el usuario tiene tiempo libre sin antes invocar `get_calendar_agenda` o `find_free_work_slots`, y nunca asumas Bogotá por defecto.\n"
    ...
    async def process_message(
        ...
        # Query user preferences if location not provided
        user_city = None
        try:
            ...
            logger.warning(f"Error consultando preferencias de usuario: {e}")
            await db.rollback()

        client = gemini_client.get_client()
        config = self._get_generate_config(user_location=user_location, user_city=user_city)
